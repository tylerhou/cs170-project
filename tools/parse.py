...
def read_input_file(path: str):
    ...
    with open(path) as input_file:
        ...
        assert input_lines[0].split()[0].isdigit(
        ), 'First line is not a valid number of tasks'
        ...
        for i in range(1, num_tasks + 1):
            ...
            assert len(
                task_parameters) == 4, 'The number of parameters in task {} is incorrect'.format(i)

            assert task_parameters[0].isdigit(
            ), 'Task {} has an invalid task id {}'.format(i, task_parameters[0])
            assert int(task_parameters[0]) == i, 'Task {} has an invalid task id {}'.format(
                i, task_parameters[0])

            assert task_parameters[1].isdigit(
            ), 'Task {} has an invalid deadline {}'.format(i, task_parameters[1])
            assert 0 < int(task_parameters[1]) <= 1440, 'Task {} has an invalid deadline {}'.format(
                i, task_parameters[1])

            assert task_parameters[2].isdigit(
            ), 'Task {} has an invalid duration {}'.format(i, task_parameters[2])
            assert 0 < int(task_parameters[2]) <= 60, 'Task {} has an invalid duration {}'.format(
                i, task_parameters[2])

            try:
                float(task_parameters[3])
            except ValueError:
                assert False, 'Task {} has an invalid non-float max benefit {}'.format(
                    i, task_parameters[3])

            decimal_checker = task_parameters[3].split('.')
            assert len(decimal_checker) == 1 or len(
                decimal_checker[1]) <= 3, 'Task {} has more than 3 decimal places in its max benefit {}'.format(i, task_parameters[3])
            assert 0 < float(task_parameters[3]) < 100.0, 'Task {} has an invalid max benefit {}'.format(
                i, task_parameters[3])

            task_id, deadline, duration, max_benefit = task_parameters
            task = Task.Task(int(task_id), int(deadline),
                             int(duration), float(max_benefit))
            tasks.append(task)
        return tasks
...
def write_input_file(path: str, tasks) -> None:
    ...
    for i in range(1, num_tasks + 1):
        ...
        task = tasks[i - 1]
        task_parameters = [str(task.get_task_id()), str(task.get_deadline()), str(
            task.get_duration()), str(task.get_max_benefit())]
        ...
        input_line = '{} {} {} {}\n'.format(str(task.get_task_id()), str(
            task.get_deadline()), str(task.get_duration()), str(task.get_max_benefit()))
...
def read_output_file(path: str):
    ...
    with open(path) as input_file:
        ...
        input_lines = [line for line in input_file.readlines() if len(
            line) > 0 and line[0] != "#"]
        ...
        for i in range(num_tasks):
            ...
            assert len(input_lines[i].split(
            )) == 1, 'Invalid number of items on line {}'.format(i + 1)

            task_id = input_lines[i].split()[0]
            assert task_id.isdigit(), 'Invalid task_id {}'.format(task_id)
            assert 0 <= int(
                task_id) <= 200, 'Invalid task_id {}'.format(task_id)
            assert task_id not in task_ids, 'task_id {} appears more than once'.format(
                task_id)
